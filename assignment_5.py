...
class Velocity:
    def __init__(self): #Initializes velocity at 0.0 (float)
       self.dx = 0.0
       self.dy = 0.0


class Ball:
    def __init__(self): #Initializes the ball at a random velocity and from a random point on the left side. 
       self.center = Point()
       self.velocity = Velocity()
       self.center.x = 0
       self.center.y = random.randint(0,SCREEN_HEIGHT)
       self.velocity.dx = random.randint(1,5)
       self.velocity.dy = random.randint (-5,-1) or random.randint (1,5)
    
    def draw(self):    #It draws the ball on the screenplay.
        arcade.draw_circle_filled(self.center.x, self.center.y,BALL_RADIUS,arcade.color.ORANGE)
        
    def advance(self): #It moves the ball according to the velocity.
        self.center.x += self.velocity.dx
        self.center.y += self.velocity.dy
                
    def bounce_horizontal(self): #It changes the direction of the ball. 
        self.velocity.dx *= -1
        
    def bounce_vertical(self):   #It changes the direction of the ball. 
        self.velocity.dy *= -1
        
    def restart(self):           #It resets the ball at a random location on the left side and at a random velocity. 
        self.center.x = 5
        self.center.y = random.randint(0,SCREEN_HEIGHT)
        self.velocity.dx = random.randint(1,5)
        self.velocity.dy = random.randint (-5,-1) or random.randint (1,5)
...
class Paddle:
    def __init__(self):          #Initilizes the paddle on the middle right side. 
        self.center = Point()
        self.center.x = 395
        self.center.y = 150
    
    def draw(self):              #It draws the paddle on the screenplay. 
        arcade.draw_rectangle_filled(self.center.x, self.center.y,PADDLE_WIDTH,PADDLE_HEIGHT,arcade.color.BLACK)

    def move_up(self):           #It moves up the paddle "move_amount" units up. 
        if self.center.y < (SCREEN_HEIGHT-(PADDLE_HEIGHT/2)):
            self.center.y += MOVE_AMOUNT
            
    def move_down(self):         #It moves up the paddle "move_amount" units down. 
